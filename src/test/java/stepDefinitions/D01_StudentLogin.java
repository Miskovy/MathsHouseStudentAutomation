...
@And("the user types {string} in the email")
    public void enter_student_email(String email){
    loginPage.emailTxt().click();
    loginPage.emailTxtHovered().sendKeys(email);
}
@And("the user types {string} in the password")
    public void enter_student_password(String password){
    loginPage.passwordTxt().click();
...
@Then("the user is able to login successfully")
    ...
    WebDriverWait wait = new WebDriverWait(Hooks.driver, Duration.ofSeconds(10));
    Assert.assertTrue(wait.until(ExpectedConditions.visibilityOf(dashboard.dashboardTitle())).isDisplayed());
}
}
